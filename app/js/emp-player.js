...
var Player = function (_VjsPlayer) {
  ...
  */
  ...
  /**
   * reset And restart form current playhead
   */
  ...
  */
  ...
  Player.prototype.startResetAndReloadTimer_ = function startResetAndReloadTimer_() {
    ...
    if (!IS_CHROMECAST || !this.hasStarted()) {
      ...
      if (this.options_.resetAndReload !== false) {
        // Disable resetAndReload
        var time = 60;
        if (this.options_.resetAndReload) {
          time = this.options_.resetAndReload > 1 ? this.options_.resetAndReload : 1; // Use resetAndReload config
        } else if (IS_CHROMECAST) {
          time = 60; // reset and reload CC every hour
        } else {
          time = 60 * 3; // reset and reload Web every 3 hour
        }

        this.resetAndReloadTimer_ = this.setTimeout(function () {
          _this4.clearResetAndReloadTimer_();
          if (_this4.isLive() && _this4.techName_ === 'EmpShaka') {
            _this4.resetAndRestartFormPlayhead();
          }
        }, time * 1000 * 60);
      }
    }
      ...
      if (this.programService && (entitlement.isStaticCachupAsLive || entitlement.isDynamicCachupAsLive) && entitlement.rwEnabled) {
        this.programService().getPreviousProgram(function (program, error) {
          if (error) {
            ...
            if (end) {
              _this5.playheadTime(program.end.getTime() - 30000);
            } else {
              _this5.playheadTime(program.start.getTime() + 1000);
            }
          }
        });
      } else {
  ...
  */
  ...
  Player.prototype.handleTechWaiting_ = function handleTechWaiting_() {
    ...
    if (this.hasStarted() && !this.ended() && !this.paused() && !this.hasClass('vjs-waiting')) {
      this.playheadMoving_(function (moving) {
        ...
        if (!moving) {
          _this6.addClass('vjs-waiting');
          _this6.trigger('waiting');
          _this6.removeWaitingClass_();
        }
      });
    }
  };
  ...
  Player.prototype.removeWaitingClass_ = function removeWaitingClass_() {
    ...
    this.one('timeupdate', function () {
      ...
      if (_this7.playheadTime() - _this7.prePlayheadTime_ > 1 || _this7.playheadTime() - _this7.prePlayheadTime_ < -1) {
        //log('playheadMoving off', true);
        _this7.prePlayheadTime_ = 0;
        _this7.removeClass('vjs-waiting');
        //edgeIELoadingBugWorkaround_(); Add if need
        if (_this7.paused()) {
          _this7.trigger(empPlayerEvents.PAUSE);
        } else {
          _this7.trigger(empPlayerEvents.PLAYING);
        }
      } else {
...
ProgramService.VERSION = '2.0.90-136';
...
EntitlementExpirationService.VERSION = '2.0.90-136';
...
EntitlementMiddleware.VERSION = '2.0.90-136';
...
AnalyticsPlugin.VERSION = '2.0.90-136';
