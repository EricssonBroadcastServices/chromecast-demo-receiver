  ...
  */
  ...
  Player.prototype.handleTechWaiting_ = function handleTechWaiting_() {
    ...
    if (this.hasStarted() && !this.ended() && !this.paused() && !this.hasClass('vjs-waiting')) {
      this.playheadMoving_(function (moving) {
        ...
        if (!moving) {
          _this5.addClass('vjs-waiting');
          _this5.trigger('waiting');
          _this5.removeWaitingClass_();
        }
      });
    }
  };
  ...
  Player.prototype.removeWaitingClass_ = function removeWaitingClass_() {
    ...
    this.one('timeupdate', function () {
      ...
      if (_this6.playheadTime() - _this6.prePlayheadTime_ > 1 || _this6.playheadTime() - _this6.prePlayheadTime_ < -1) {
        //log('playheadMoving off', true);
        _this6.prePlayheadTime_ = 0;
        _this6.removeClass('vjs-waiting');
        //edgeIELoadingBugWorkaround_(); Add if need
        if (_this6.paused()) {
          _this6.trigger(empPlayerEvents.PAUSE);
        } else {
          _this6.trigger(empPlayerEvents.PLAYING);
        }
      } else {
...
ProgramService.VERSION = '2.0.88-126';
...
EntitlementExpirationService.VERSION = '2.0.88-126';
...
EntitlementMiddleware.VERSION = '2.0.88-126';
...
AnalyticsPlugin.VERSION = '2.0.88-126';
